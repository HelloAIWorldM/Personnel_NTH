  ...
  return (
    ...
    >
      <div
        ...
        className="bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 rounded-2xl max-w-2xl w-full max-h-[92vh] flex flex-col shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden"
      >
        {/* Modal Header */}
        ...
        <div className="p-3 sm:p-5 overflow-y-auto space-y-5">
          ...
          <div>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 mb-2">
              ...
              <button
                ...
                className="flex items-center gap-1 text-[11px] font-semibold text-slate-500 dark:text-slate-400 hover:text-red-600 dark:hover:text-red-400 transition-colors py-1"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              {COLOR_PRESETS.map((preset) => (
                <button
                  ...
                  className="p-2.5 sm:p-3 text-left border border-slate-200 dark:border-slate-700 hover:border-purple-400 dark:hover:border-purple-500 rounded-xl hover:bg-purple-50/30 dark:hover:bg-purple-950/20 transition-all group"
                >
                  <div className="text-xs font-bold text-slate-800 dark:text-slate-200 group-hover:text-purple-700 dark:group-hover:text-purple-300 mb-0.5">
                    {preset.name}
                  </div>
                  <div className="text-[10px] text-slate-500 dark:text-slate-400 line-clamp-1 mb-1.5">
                    {preset.desc}
                  </div>
                  {/* Color dots preview */}
          ...
          <div>
            <div className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wide mb-2.5">
              ...
              {CLASS_LIST.map((cls) => {
                ...
                return (
                  <div
                    ...
                    className="flex items-center justify-between p-2.5 border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 rounded-xl bg-white dark:bg-slate-800/80 gap-2"
                  >
                    {/* Left: Preview Pill */}
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      ...
                      <label
                        className="relative cursor-pointer w-9 h-9 rounded-xl overflow-hidden border border-slate-300 dark:border-slate-600 shadow-2xs hover:scale-105 active:scale-95 transition-transform flex items-center justify-center shrink-0"
                      ...
                      {isCustomized && (
                        <button
                          ...
                          className="p-1.5 text-slate-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors min-h-[36px] min-w-[36px] flex items-center justify-center"
                        >
        ...
        {/* Modal Footer */}
        <div className="p-3 sm:p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-850 flex flex-col sm:flex-row items-center justify-between gap-2">
          ...
          <button
            type="button"
            onClick={onClose}
            className="w-full sm:w-auto flex items-center justify-center gap-1.5 px-4 py-2 bg-slate-900 hover:bg-slate-800 dark:bg-indigo-600 dark:hover:bg-indigo-700 text-white text-xs font-bold rounded-xl transition-all shadow-xs min-h-[40px]"
          >
